...
\begin{frame}{Self-Edit Distance: Intuition}
    \begin{columns}
        ...
        \onslide<3->{
            Say $X[i\dd j)=P=Y[i'\dd j')$ for $|i-i'|\le k$.
...
\end{frame}
 ...
 \begin{frame}<4->{Self-Edit Distance}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item  If $X[i\dd j)=P=Y[i'\dd j')$ with $|i-i'|\le k$ and $\sed(P)> 3k$,
...
\begin{frame}{Open Problems}
    ...
    \end{enumerate}
      ...
      \node<5->[text=MPIgreen] at ([xshift=2.7cm,yshift=.3cm]current page.center){\Huge Thank you!};
      \end{tikzpicture}%
